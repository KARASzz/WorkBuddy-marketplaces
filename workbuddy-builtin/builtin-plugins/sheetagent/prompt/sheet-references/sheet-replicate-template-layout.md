...
```javascript
const existingMerges = targetSheet.getMergedRanges();
if (existingMerges.length > 0) {
  ...
  for (const mr of existingMerges) {
    if (mr.getRow() <= srcRows && mr.getColumn() <= srcCols) {
      mr.breakApart();
    }
  }
}
...
| Structure | Merged cells | `sheet.getMergedRanges()` | `range.merge()` / `mergeAcross()` / `mergeVertically()` |
...
| `getMergedRanges` | `getMergedRanges(startRow?, startCol?, numRows?, numCols?)` | Optional params are **1-based**; defaults to entire sheet |

#### `getMergedRanges` Return Value

Returns `Range[]` — one Range per merged region (a merge that merely **overlaps** the queried
box is included). Read its position with the usual Range getters, all **1-based**:
- `getRow()` / `getColumn()`: start row / column of the merged region
- `getNumRows()` / `getNumColumns()`: size of the merged region
- `getA1Notation()`: e.g. `"A1:C2"` — also what `JSON.stringify()` prints, so logging the
  array gives readable coordinates
- Act on it directly: `mergedRange.breakApart()` / `.getValue()`
...
- `getLastRow()` / `getLastColumn()` return the last row/column **with content** (0 on an empty sheet) — use them to bound your copy loops. The sheet's **grid size** is a separate pair, `getMaxRows()` / `getMaxColumns()`; don't substitute one for the other
...
const mergedRanges = templateSheet.getMergedRanges(); // returns Range[] (1-based getters)
for (const mr of mergedRanges) {
  const target = targetSheet.getRange(mr.getRow(), mr.getColumn(), mr.getNumRows(), mr.getNumColumns());
  const vals = target.getValues();
  const safe = vals.every((row, r) => row.every((v, c) => (r === 0 && c === 0) || !v));
  if (safe) target.merge();
}
