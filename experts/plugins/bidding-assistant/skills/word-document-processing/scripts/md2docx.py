...
DEFAULT_REFERENCE = SHARED_DIR / "word-templates" / "template.docx"
DEFAULT_FILTER = SHARED_DIR / "pandoc-lua-filters" / "markdown-to-docx.lua"
...
def resolve_resource(path_text: str) -> Path:
    ...
    for candidate in (path, SHARED_DIR / "word-templates" / path_text, SCRIPT_DIR / path_text):
        if candidate.exists():
            return candidate
    return path
