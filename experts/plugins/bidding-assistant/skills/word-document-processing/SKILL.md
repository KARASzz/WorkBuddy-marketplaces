...
> 以下命令需在 skill 根目录下执行。OOXML 工具包已迁移至共享依赖 `../_shared/ooxml/`。

`python ../_shared/ooxml/scripts/unpack.py <office_file> <output_directory>`
...
# 1. pandoc 转换
pandoc input.md \
  --reference-doc=../_shared/word-templates/template.docx \
  --lua-filter=../_shared/pandoc-lua-filters/markdown-to-docx.lua \
  -o draft.docx
...
- 模板未给 Heading 1 配置 `pageBreakBefore`，章节标题不会自动分页——**如需"每个章节独立成页"，必须在每个章节标题前显式加分页符**；如希望所有 H1 自动分页，可在 `../_shared/word-templates/template.docx` 的 Heading 1 样式里加 `<w:pageBreakBefore/>`
...
> 详细工作流请查阅 [`advanced-editing.md`](../_shared/advanced-editing.md)。

| 场景 | 说明 | 入口 |
|------|------|------|
| 编辑现有文档 | 使用 Document 库修改已有 .docx | [编辑现有 Word 文档](../_shared/advanced-editing.md#复杂：编辑现有-word-文档document-库) |
| Redlining 修订 | 带修订跟踪的文档审阅工作流 | [Redlining 修订工作流](../_shared/advanced-editing.md#复杂：redlining-修订工作流文档审阅) |
...
- 使用前**必须**完整阅读 [`ooxml.md`](../_shared/ooxml.md)
