...
- [Document Library (Python)](#document-library-python) - Recommended approach for OOXML manipulation with automatic infrastructure setup, including tracked changes and XML pattern reference
...
**Run your script with PYTHONPATH** set to this skill's directory:
```bash
...
`get_node()` requires exactly one match (raises `ValueError` if 0 or multiple). For ambiguous text, use `get_nodes()` first to see all matches, then pick by index or add `line_number` to disambiguate.

**For text that spans multiple runs** (common in Word-generated documents where text is split into single-character `<w:r>/<w:t>` fragments), use `find_text()` instead. It searches the **concatenated text** of entire paragraphs, so it can find text regardless of how it's fragmented across runs.

```python
# By text content (single run)
...
# Get ALL matches first (useful when text appears multiple times)
nodes = doc["word/document.xml"].get_nodes(tag="w:r", contains="2026")
if len(nodes) == 1:
    node = nodes[0]
elif len(nodes) > 1:
    # Pick specific one by index, or use more specific contains
    node = nodes[1]  # or nodes[0], etc.
else:
    raise ValueError("No matches")
```

#### find_text() — 搜索跨 run 的碎片化文本

Word 生成的文档常把文本拆成单字符 run（如 "招标人" 拆成 `招`/`标`/`人` 三个 `<w:r>`）。`get_node(contains=...)` 只搜索单个 run 内的拼接文本，搜不到跨 run 的文本。`find_text()` 搜索段落的**完整拼接文本**，无视碎片化：

```python
# 搜索跨 run 的文本（返回所有含该文本的段落）
results = doc["word/document.xml"].find_text("电梯专用摄像机")
for r in results:
    print(f"Line {r['line']}: {r['text'][:60]}")
    # r['node']: 段落 DOM 元素
    # r['text']: 段落完整拼接文本
    # r['runs']: 段落内所有 run 的列表 [(node, text, line), ...]
```

#### replace_text_in_paragraph() — 高层文本替换（自动处理碎片化）

不用手动处理 XML 标签，直接声明"在某段落中把 A 改为 B"，自动生成 `<w:del>`/`<w:ins>` 修订标记：

```python
# 找到段落 + 替换文本（一步到位，自动处理跨 run 的碎片化文本）
results = doc["word/document.xml"].find_text("电梯专用摄像机")
if results:
    success = doc["word/document.xml"].replace_text_in_paragraph(
        results[0]['node'],      # 段落元素
        "电梯专用摄像机",          # 原文本
        "网络摄像机"               # 新文本
    )
    # 自动生成 <w:del>旧文本</w:del><w:ins>新文本</w:ins>
```

**优势**：
- 无需关心文本被拆成几个 run
- 无需手动构造 `<w:del>`/`<w:ins>` XML
- 无需用 `line_number` 定位（直接用段落引用）
- 一次调用完成"查找+替换+修订标记"
...
以下为构造替换字符串时的 XML 参考模式，实际操作应通过 Document 库完成。
